...
class TestExamination(TransactionCase):
    ...
    def submit_claim(self, invoice_id):
        ...
        session_id = self.env['session.session'].search(
                [('user_id', '=', user_id.id), ('state', '!=', 'closed_posted')]).id
            ...
            if responsepayload:
                ...
                if lines and lines[0] and lines[0].startswith('H'):
                    ...
                    if total_approved_price or status_claim_level == 'Claim Accepted for delivery' or status_claim_level == 'Claim Accepted for processing':
                        print("\n\n=======> in if for total approved price.")
                        invoice.action_date_assign()
                        print("\n\n=======> action_invoice_open done.")
                        vals = {}
                        accquire_id = self.env['payment.acquirer'].search([('name', '=', 'Mediswitch Payment Gateway'),('company_id','=',invoice.company_id.id)], limit=1)
                        print("\n\n=======> accquire_id", accquire_id)
                        vals.update({
