...
class CustomerActivityStatement(models.AbstractModel):
    ...
    _description = 'Report Customer Activity Statement '
    ...
    def _get_account_initial_balance(self, company_id, partner_ids,
        ...
        date_start = datetime.strptime(
            str(date_start), DEFAULT_SERVER_DATE_FORMAT).date()
    ...
    def _get_account_display_lines(self, company_id, partner_ids, date_start,
        ...
        date_end = datetime.strptime(
            str(date_end), DEFAULT_SERVER_DATE_FORMAT).date()
        # pylint: disable=E8103
        self.env.cr.execute("""WITH Q1 AS (%s), Q2 AS (%s)
    ...
    def _get_report_values(self, docids, data):
        ...
        if not data.get('company_id'):
            wiz_id = self.env['customer.activity.statement.wizard'].browse(docids[0])
        company_id = data.get('company_id',False) if data.get('company_id',False) else wiz_id.company_id.id
        partner_ids = data.get('partner_ids',False) if data.get('partner_ids',False) else self._context.get('partner_ids')
        date_start = data.get('date_start',False) if data.get('date_start',False) else wiz_id.date_start
        date_end = data.get('date_end',False) if data.get('date_end',False) else wiz_id.date_end
        account_type = data.get('account_type',False) if data.get('account_type',False) else wiz_id.account_type
        ...
        if data.get('show_aging_buckets',False) if data.get('show_aging_buckets',False) else wiz_id.show_aging_buckets:
        ...
        print("\n\n\n lines to display--->", lines_to_display)
        return {
            'doc_ids': partner_ids,
            ...
            'Show_Buckets': data.get('show_aging_buckets') if data.get('show_aging_buckets') else wiz_id.show_aging_buckets,
            'Filter_non_due_partners': data.get('filter_non_due_partners') if data.get('filter_non_due_partners') else wiz_id.filter_partners_non_due,
