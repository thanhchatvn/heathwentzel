...
class MassSms(models.Model):
    ...
    def _compute_statistics(self):
        ...
        for record in self:
            ...
            errors = len(SmsMessage.search([('mass_sms_id', '=', record.id), (
            'status_code', 'in', ('001', '002', '005', '006', '007', '009', '010', '011', '012', '013', '014'))]))
    ...
    def _process_mass_sms_queue(self):
        mass_sms = self.search(
            [('state', 'in', ('queue', 'sending')), '|', ('scheduled_date', '<', fields.Datetime.now()),
    ...
    def send_sms(self):
        ...
        for participant in participants[:self.batch_size]:
            if participant.partner_id.mobile:
                render_msg = self.env['sms.template'].render_template(self.sms_template_id.template_body,
    ...
    def open_received_sms_message(self):
        ...
        messages = self.env['sms.message'].search(
            [('mass_sms_id', '=', self.id), ('status_code', 'in', ('003', '004'))])
    ...
    def open_error_sms_message(self):
        self.ensure_one()
        messages = self.env['sms.message'].search([('mass_sms_id', '=', self.id), (
        'status_code', 'in', ('001', '002', '005', '006', '007', '009', '010', '011', '012', '013', '014'))])
        action = self.env.ref('inuka_sms.action_sms_message_form_outbound').read()[0]
        action['domain'] = [('id', 'in', messages.ids)]
        action['view_mode'] = 'tree,form'
    ...
    def send_sms_reply(self):
        ...
        for message in self:
            ...
            if shortcode and shortcode.member_required and message.model_id.model == 'res.partner':
                ...
                if partner.mobile:
                    render_msg = self.env['sms.template'].render_template(shortcode.sms_template_id.template_body,
    ...
    def send_entity(self):
        ...
        if self.sms_template_id:
            ...
            if self.env.context.get('from_cron'):
                number_id = self.env['sms.number'].search([('company_id', '=', self.env.user.company_id.id)])
                if number_id.account_id.account_gateway_id.gateway_model_name == "sms.gateway.clickatell":
                    my_sms = number_id.account_id.send_message(self.from_mobile_id.mobile_number, self.to_number,
                                                                         self.sms_content.encode('utf-8'), self.model,
                                                                         self.record_id, self.media_id)
                else:
                    sms_template_id_2 = self.env['sms.template'].browse(int(self.sms_template_id))
                    sms_rendered_content = self.env['sms.template'].render_template(sms_template_id_2.template_body,
                                                                                    sms_template_id_2.model_id.model,
                                                                                    self.record_id)
                    sms_message = self.env['sms.api'].smsapi(number_id.account_id, self.to_number,
                                                             sms_rendered_content,
                                                             self.record_id, self.model, self.from_mobile_id, self.media_id,
                                                             self.media_filename)
            else:
                number_id = self.env['sms.number'].search([('company_id', '=', self.env.user.company_id.id)])
                if number_id.account_id.account_gateway_id.gateway_model_name == "sms.gateway.clickatell":
                    ...
                    sms_message = self.env['sms.api'].smsapi(number_id.account_id, self.to_number, self.sms_content.encode('utf-8'),
                                                             self.record_id, self.model, self.from_mobile_id, self.media_id,
                                                             self.media_filename)
            if my_sms and len(my_sms['messages']) > 0:
                pass
            else:
                if my_sms:
            ...
            if not sms_message:
                sms_message = self.env['sms.message'].create({
                    ...
                    'status_string': my_sms['error'] if my_sms and 'error' in my_sms.keys() else False,
                    'direction': 'O',
                    'message_date': datetime.utcnow(),
                    'status_code': my_sms['errorCode'] if my_sms and 'errorCode' in my_sms.keys() else False,
                    'sms_gateway_message_id': my_sms[
                        'apiMessageId'] if my_sms and 'apiMessageId' in my_sms.keys() else False,
            ...
            raise Warning(_("Please Select Template"))
...
class Smsapi(models.Model):
    ...
    def smsapi(self, account_id, to_number, sms_content, record_id, model, from_mobile_id, media_id, media_filename):
        authorization_key = account_id.client_id + ':' + account_id.secret_key
        ...
        response = requests.request("GET", account_id.authentication_url, headers=headers)
        Authorization = 'Bearer ' + str(response.json().get('token'))
        sendRequest = {
            "messages": [{"content": sms_content, "destination": to_number}]
        }
        sendResponse = requests.post(account_id.message_url,
                                     headers={'Authorization': Authorization},
                                     json=sendRequest)
        my_sms = sendResponse.json()
        if my_sms.get('errors'):
            raise Warning(
                _(my_sms.get('errors')[0].get('errorMessage') + "\n(" + my_sms.get('errors')[0].get('errorCode') + ")"))
        else:
            my_model = self.env['ir.model'].search([('model', '=', model)])
        sms_message = self.env['sms.message'].create({
            'record_id': record_id,
            'model_id': my_model[0].id,
            'account_id': from_mobile_id.account_id.id if from_mobile_id else False,
            'from_mobile': from_mobile_id.mobile_number if from_mobile_id else False,
            'to_mobile': to_number if to_number else False,
            'sms_content': sms_content,
            'status_string': my_sms.get('errors')[0].get('errorMessage') if my_sms and my_sms.get(
                'errors') else "Message sent successfully.",
            'direction': 'O',
            'message_date': datetime.utcnow(),
            'status_code': my_sms.get('errors')[0].get('errorCode') if my_sms and my_sms.get('errors') else False,
            'sms_gateway_message_id': my_sms.get('eventId') if my_sms and my_sms.get('eventId') else False,
            'by_partner_id': self.env.user.partner_id.id,
            'mass_sms_id': self.env.context.get('mass_sms_id'),
        })
        sms_subtype = self.env['ir.model.data'].get_object('sms_frame', 'sms_subtype')
        attachments = []
        self.env[model].search([('id', '=', record_id)]).message_post(body=sms_content,
                                                                      subject="SMS Sent",
                                                                      message_type="comment",
                                                                      subtype_id=sms_subtype.id,
                                                                      attachments=attachments)
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('active_model') == 'res.partner':
            result['recipients'] = ", ".join(
                [self.env['res.partner'].browse(int(id)).mobile for id in self.env.context.get('active_ids')])
        return result
